...
// updateNeighbors();
  ...
  scan_button.on('pointerdown', (event) => {
  ...
  map_button.position.set(app.screen.width *5/ 6, (app.screen.height/4) +100);
  scan_button.position.set(app.screen.width *5/ 6, (app.screen.height/4) +200);
  
  diver.position.set(area_arr[currentArea].center.x, area_arr[currentArea].center.y);
  ...
  map_button.on('pointerdown', (event) => {
      console.log('Clicked or touched!');
